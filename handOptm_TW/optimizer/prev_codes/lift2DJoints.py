...
def lift2Dto3DMultiView(MVpred, initPose, refPose, ref, weights=None, camMat=None, relDepGT=None, isOpenGLCoord=False, outDir=None):
    ...
    :param weights: (num_views)x1 array ~ weight per each view from view-point selection
    :param relDepGT: need to implement
    :param isOpenGLCoord:
    :return: always in opencv
    '''
    ### initialize ###
    loss = {}
    numViews = MVpred.shape[0]
    if weights is None:
        weights = [np.ones((1), dtype=np.float32)]*numViews

    ### set variables to optimize ###
    outputPose = ch.array(initPose)
    freeVars = [outputPose]
    ...
    ### relative depth loss ###
    """
    # compare prediced 2.5D depth difference with acquired depthmap values on projected hand joint 2D position
    if relDepGT is not None:
        for i in range(numViews):
            relDepPred = mList[i].J_transformed[:,2] - mList[i].J_transformed[0,2]
            loss['relDep_%d'%(i)] = (relDepPred - relDepGT[i]) * weights[:,0] * 5e1
