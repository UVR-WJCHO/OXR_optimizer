...
def showHandJoints_cv(imgInOrg, gtIn, estIn=None, filename=None, upscale=1, lineThickness=3):
    '''
    Utility function for displaying hand annotations
    :param imgIn: image on which annotation is shown
    :param gtIn: ground truth annotation
    ...
    # Set color for each finger
    joint_color_code = [[139/255.0, 53/255.0, 255/255.0],
                        [0/255.0, 56/255.0, 255/255.0],
                        [43/255.0, 140/255.0, 237/255.0],
                        [37/255.0, 168/255.0, 36/255.0],
                        [147/255.0, 147/255.0, 0/255.0],
                        [70/255.0, 17/255.0, 145/255.0]]
    ...
    for joint_num in range(gtIn.shape[0]):
        ...
        if joint_num in [0, 4, 8, 12, 16]:
            if PYTHON_VERSION == 3:
                joint_color = list(map(lambda x: x + 35/255.0 * (joint_num % 4), joint_color_code[color_code_num]))
            else:
                joint_color = map(lambda x: x + 35/255.0 * (joint_num % 4), joint_color_code[color_code_num])
    ...
    for limb_num in range(len(limbs)):
        ...
        if length < 150 and length > 5:
            ...
            color_code_num = limb_num // 4
            if PYTHON_VERSION == 3:
                limb_color = list(map(lambda x: x + 35/255.0 * (limb_num % 4), joint_color_code[color_code_num]))
            else:
                limb_color = map(lambda x: x + 35/255.0 * (limb_num % 4), joint_color_code[color_code_num])
